  ...
  const actions: ActionItem[] = [
    {
      ...
      title: siteSettings.maintenanceMode ? 'إيقاف وضع الصيانة العام (Site Normal)' : 'تفعيل وضع الصيانة العام (Maintenance Mode)',
      subtitle: siteSettings.maintenanceMode ? 'الوضع مفعّل حالياً: اضغط لإلغائه وإعادة التنزيلات للزوار' : 'اضغط لتجميد التنزيلات للزوار وإظهار شريط التنبيه',
      icon: ShieldAlert,
      iconColor: siteSettings.maintenanceMode ? 'text-rose-400' : 'text-amber-400',
      badge: siteSettings.maintenanceMode ? 'مفعّل الآن 🚨' : 'متوقف 🟢',
      badgeBg: siteSettings.maintenanceMode ? 'bg-rose-500/20 text-rose-300 border-rose-500/30' : 'bg-emerald-500/20 text-emerald-300 border-emerald-500/30',
    ...
    {
      id: 'jump_seo_platforms',
      category: 'admin_jump',
      title: 'محرر صفحات الـ SEO والـ Metatags الثابتة',
      subtitle: 'تعديل نصوص وعناوين الـ Metatags والـ Keywords لـ (TikTok, FB, YT, IG)',
      icon: Layers,
      iconColor: 'text-purple-400',
      badge: 'SEO Center',
      badgeBg: 'bg-purple-500/20 text-purple-300 border-purple-500/30',
      action: () => {
        onOpenAdminTab('platforms');
        onClose();
      },
    },
    {
      id: 'jump_site_settings',
      category: 'admin_jump',
      title: 'الإعدادات العامة والربط (النظام والمحتوى)',
      subtitle: 'تخصيص اسم الموقع، الشعار، العبارة الت مرحيبية، وربط GA4 وAdSense',
      icon: Settings,
      iconColor: 'text-indigo-400',
      action: () => {
        onOpenAdminTab('site_settings');
        onClose();
      },
    },
    {
      id: 'jump_pages',
      category: 'admin_jump',
      title: 'إدارة الصفحات الهبوط (Pages CMS Center)',
      subtitle: 'إنشاء وتعديل الصفحات الفرعية وصفحات الشروط والسياسات',
      icon: FileText,
      iconColor: 'text-sky-400',
      action: () => {
        onOpenAdminTab('pages');
        onClose();
      },
    },
    {
      id: 'jump_seo_center',
      category: 'admin_jump',
      title: 'مركز الـ SEO التقني والمؤشرات الشاملة',
      subtitle: 'فحص جودة الأرشفة والروابط وتوليد خريطة الموقع Sitemap.xml',
      icon: Globe,
    ...
    },
    {
      id: 'jump_theme',
      category: 'admin_jump',
      title: 'استوديو المظهر والألوان (Theme Builder)',
      subtitle: 'تخصيص الهيدر، ألوان التدرج، حواف الكروت وأنماط الأزرار',
      icon: Palette,
      iconColor: 'text-pink-400',
      action: () => {
        onOpenAdminTab('theme');
        onClose();
      },
    },
    {
      id: 'jump_security',
      category: 'admin_jump',
      title: 'الأمان وحظر عناوين الـ IP (Security Center)',
      subtitle: 'حظر السبام، تفعيل جدار الحماية وتحديد معدل الطلبات Rate Limit',
      icon: Shield,
      iconColor: 'text-rose-400',
      action: () => {
        onOpenAdminTab('users_security');
        onClose();
      },
    },
    {
      id: 'jump_ai_suite',
      category: 'admin_jump',
      title: 'جناح الذكاء الاصطناعي (AI Suite Center)',
      subtitle: 'توليد المقالات والـ SEO تلقائياً باستخدام Gemini AI',
      icon: Sparkles,
      iconColor: 'text-amber-400',
      action: () => {
        onOpenAdminTab('ai_suite');
        onClose();
      },
    },
    {
      id: 'jump_ads',
      category: 'admin_jump',
      title: 'إدارة الإعلانات ومواضع AdSense',
      subtitle: 'توزيع البنرات الإعلانية واختبار أداء النقاط الإعلانية',
      icon: DollarSign,
      iconColor: 'text-emerald-400',
      action: () => {
        onOpenAdminTab('ads');
    ...
    },
    {
      id: 'jump_analytics',
      category: 'admin_jump',
      title: 'تحليلات الأداء وإحصائيات الزوار',
      subtitle: 'مراقبة عدد التحميلات المباشرة، الدولة، ونوع الجهاز',
      icon: BarChart3,
      iconColor: 'text-blue-400',
      action: () => {
        onOpenAdminTab('analytics');
        onClose();
      },
    },
    {
