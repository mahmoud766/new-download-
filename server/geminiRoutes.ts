...
router.post('/chat', async (req: Request, res: Response) => {
  try {
    ...
    return res.status(500).json({
      ...
      text: 'Sorry, I hit an issue reaching the AI engine. Please verify your Gemini API key or try again.'
    });
  }
});
...
router.post('/search', async (req: Request, res: Response) => {
  try {
    ...
    return res.status(500).json({
      ...
      text: 'Unable to perform Google Search grounding at this moment.'
    });
  }
});
...
router.post('/thinking', async (req: Request, res: Response) => {
  try {
    ...
    return res.status(500).json({
      ...
      text: 'Could not process query in high-thinking mode.'
    });
  }
});
...
router.post('/veo/generate', async (req: Request, res: Response) => {
  try {
    ...
    // Fallback sample video rendering if key or model quota requires preview fallback
...
router.post('/image/generate', async (req: Request, res: Response) => {
  try {
    ...
    // Unsplash dynamic AI placeholder fallback
...
router.post('/music/generate', async (req: Request, res: Response) => {
  try {
    ...
    // High quality audio clip sample output
    ...
    return res.status(500).json({ error: err.message || 'Failed to generate music clip' });
  }
});
...
router.post('/ai/generate', async (req: Request, res: Response) => {
  try {
    ...
    const { tool, topic } = req.body;
    let fallbackText = `توليد تلقائي للموضوع: ${topic}`;
    if (tool === 'meta') {
      ...
      fallbackText = `# طريقة ${topic}\n\nيقدم موقع OmniFetch أسهل وأسرع طريقة لتنزيل مقاطع ${topic} بضغط زرة واحدة.`;
    }
